...
try {
    ...
    */
    ...
    foreach ($pvs as $pv) {
        $codPv = $pv['Nº'];
        $nombrePv = $pv['Punto de venta'];
        $nuevoNombre = $nombrePv . ', ' . $codPv;
        $pvVector = [];
        $id_con_prefijo = $pv['ID de registro'];
        $id_numerico = preg_replace('/\D/', '', $id_con_prefijo); // Eliminar todo lo que no sea un dígito
        $pvVector['data'][0]['id'] = $id_numerico;
        $pvVector['data'][0]['Name'] =  $nuevoNombre;
        $pvJson = json_encode($pvVector);
        $crm->actualizar("actualizarPdvs", $pvJson);
        if ($crm->estado) {
            echo '<p style="color:green;display:flex;flex-direction:column;justify-content:center;align-items:center;width:100%">' . $i . '</p>';
            echo '<p style="color:green;display:flex;flex-direction:column;justify-content:center;align-items:center;width:100%">PV ACTUALIZADO: ' . $codPv . '</p>';
            ...
            echo '<p style="color:red;display:flex;flex-direction:column;justify-content:center;align-items:center;width:100%">ERROR!!! EN EL PV: ' . $codPv . '</p>';
            print_r($crm->respuesta);
            scrollUpdate();
            @ob_flush();
            flush();
