...
function db(database, stores) {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = (event) => {
      console.error(
        `Error en la función db al tratar de abrir o crear la base de datos ${database}: ${event.target.error}`
      );
...
function existDb(dataBase) {
  return new Promise((resolve, reject) => {
    ...
    indexedDB
      ...
      .catch((error) => {
        console.error(
          `Error en la función existDb al verificar la existencia de la base de datos ${dataBase}: ${error.message}`
        );
        reject(error);
      });
  });
...
function existStore(db, almacen) {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = (event) => {
      console.error(
        `Error en la función existStore al abrir la base de datos ${db} para verificar si el almacen ${almacen} existe: ${event.target.error}`
      );
      reject(event.target.error);
    };
  });
...
function agregarDato(database, store, data) {
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = (event) => {
      ...
      clearRequest.onsuccess = (clearEvent) => {
        ...
        requestAgregar.onerror = (event) => {
          console.error(
            `Error en la función agregarDato al agregar los datos al almacén ${store}: ${event.target.error}`
          );
          reject(event.target.error);
        };
      };
      clearRequest.onerror = (event) => {
        console.error(
          `Error en la función agregarDato al limpiar el almacén ${store} para ingresar los nuevos datos: ${event.target.error}`
        );
        reject(event.target.error);
      };
    };
    request.onerror = (event) => {
      console.error(
        `Error en la función agregarDato al abrir la base de datos ${database} para ingresar los nuevos datos: ${event.target.error}`
      );
      reject(event.target.error);
    };
  });
...
function deleteDB(database) {
  return new Promise((resolve, reject) => {
    ...
    request.onblocked = () => {
      console.warn(
        `La eliminación de la base de datos "${database}" está bloqueada. Asegúrate de cerrar todas las conexiones a esta base de datos.`
      );
      reject(
        new Error(
          `La eliminación de la base de datos "${database}" está bloqueada.`
        )
      );
    };
  });
