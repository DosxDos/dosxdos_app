...
async function loadFirebase() {
  try {
    ...
    onMessage(messaging, (payload) => {
      ...
      const numeroDeNotificacionesActuales =
        $numeroDeNotificacionesActuales.innerHTML;
      const numeroDeNotificacionesActualesInt = parseInt(
        numeroDeNotificacionesActuales
      );
      $numeroDeNotificacionesActuales.innerHTML =
        numeroDeNotificacionesActualesInt + 1;
      ...
      if (Notification.permission === "default") {
        Notification.requestPermission().then((permission) => {
          if (permission === "granted") {
            const notificacionSistemaOperativo = new Notification(title, {
              body,
              icon,
            });
...
function notificarWebApp() {
  try {
    ...
    if (dataNativa != null) {
      ...
      const $numeroDeNotificacionesActuales = document.getElementById(
