...
const CACHE_NAME = "dosxdos105",
  urlsToCache = [
    ...
    "http://localhost/dosxdos_app/js/utils/notificaciones.js",
    "http://localhost/dosxdos_app/js/utils/notificaciones2.js",
    "http://localhost/dosxdos_app/js/utils/loadFirebase.js",
        ...
        return Promise.all(
          urlsToCache.map((url) => {
            return fetch(url, { cache: "no-store" }).then((response) => {
              if (!response.ok) {
                console.error(
                  `Error al intentar agregar un elemento a la caché: Failed to fetch ${url}`
                );
              }
        ...
        if (navigator.onLine) {
          try {
            ...
            if (urlsToUpdate.includes(event.request.url)) {
              ...
              console.log(
                "Respuesta del servidor y se actualizó en caché: " +
                  event.request.url
              );
              return response;
            } else {
            ...
            console.error(
              "Error en la solicitud: " + event.request.url + "-" + error
            );
          }
        } else {
